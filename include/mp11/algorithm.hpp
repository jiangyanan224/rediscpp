#ifndef MP11_ALGORITHM_HPP_INCLUDED
#define MP11_ALGORITHM_HPP_INCLUDED
...
#include <mp11/set.hpp>
#include <mp11/integral.hpp>
#include <mp11/utility.hpp>
#include <mp11/function.hpp>
#include <mp11/detail/mp_count.hpp>
#include <mp11/detail/mp_plus.hpp>
#include <mp11/detail/mp_map_find.hpp>
#include <mp11/detail/mp_with_index.hpp>
#include <mp11/detail/mp_fold.hpp>
#include <mp11/detail/mp_min_element.hpp>
#include <mp11/detail/mp_copy_if.hpp>
#include <mp11/detail/mp_remove_if.hpp>
...
#if MP11_WORKAROUND(MP11_MSVC, == 1900) || MP11_WORKAROUND(MP11_GCC, < 40800)
...
template <template <class...> class F, template <class...> class L1, class... T1, template <class...> class L2, class... T2, template <class...> class L3,
          class... T3, template <class...> class L4, class... T4, class... L>
...
template <class L, std::size_t N>
using mp_drop_c = mp_assign<L, typename detail::mp_drop_impl<mp_rename<L, mp_list>, mp_repeat_c<mp_list<void>, N>, mp_bool<N <= mp_size<L>::value>>::type>;
...
struct mp_from_sequence_impl<S<U, J...>, F, false>
{
    using type = mp_list_c<U, (F::value + J)...>;
};

template <template <class T, T... I> class S, class U, U... J, class F>
struct mp_from_sequence_impl<S<U, J...>, F, true>
{
...
using mp_from_sequence = typename detail::mp_from_sequence_impl<S, F, (F::value == 0)>::type;
...
using mp_iota = mp_from_sequence<make_integer_sequence<typename std::remove_const<decltype(N::value)>::type, N::value>, F>;
...
#if defined(MP11_HAS_TYPE_PACK_ELEMENT)
    ...
    struct detail
    {
        struct mp_nth_element_impl_cuda_workaround
        {
            using type = mp_cond<
                ...
                mp_bool<(I < N1)>, mp_nth_element_impl<L1, I, P>, mp_bool<(I == N1)>, mp_identity<T1>, mp_true, mp_nth_element_impl<L2, I - N1 - 1, P>
...
template <template <class...> class L, class T1, class T2, class T3, class T4, class T5, class T6, class T7, class T8, class T9, class T10, class... T, class V,
...
MP11_CONSTEXPR F mp_for_each_impl(mp_list<T...>, F &&f)
{
...
template <class F>
MP11_CONSTEXPR F mp_for_each_impl(mp_list<>, F &&f)
{
...
MP11_CONSTEXPR mp_if_c<mp_size<L>::value <= 1024, F> mp_for_each(F &&f)
{
    return detail::mp_for_each_impl(mp_rename<L, mp_list>(), std::forward<F>(f));
}

template <class L, class F>
MP11_CONSTEXPR mp_if_c<mp_size<L>::value >= 1025, F> mp_for_each(F &&f)
{
...
template <class L, class F>
MP11_CONSTEXPR F mp_for_each(F &&f)
{
    return detail::mp_for_each_impl(mp_rename<L, mp_list>(), std::forward<F>(f));
}
...
template <class L, class N, class Q>
using mp_sliding_fold_q = typename detail::mp_sliding_fold_impl<mp_bool<(mp_size<L>::value >= N::value)>, L, N, Q>::type;
...
#endif  // #ifndef MP11_ALGORITHM_HPP_INCLUDED
