#ifndef MP11_DETAIL_MP_WITH_INDEX_HPP_INCLUDED
#define MP11_DETAIL_MP_WITH_INDEX_HPP_INCLUDED
...
#define MP11_UNREACHABLE_DEFAULT \
    default:                     \
...
struct mp_with_index_impl_
{
    ...
    static constexpr decltype(std::declval<F>()(std::declval<mp_size_t<0>>())) call(std::size_t i, F &&f)
    {
        if (i < N / 2)
        {
...
inline constexpr decltype(std::declval<F>()(std::declval<mp_size_t<0>>())) mp_with_index(std::size_t i, F &&f)
{
    assert(i < N);
...
#undef constexpr
#undef MP11_UNREACHABLE_DEFAULT

}  // namespace mp11

#endif  // #ifndef MP11_DETAIL_MP_WITH_INDEX_HPP_INCLUDED
