...
#ifndef REDIS_CONFIG_HPP
#define REDIS_CONFIG_HPP
...
struct config
{
    ...
    std::string clientname = "Redis";

    /// Database that will be passed to the [SELECT](https://redis.io/commands/hello/) command.
    std::optional<int> database_index = 0;

    /// Message used by the health-checker in `redis::connection::async_run`.
    std::string health_check_id = "Redis";

    /// Logger prefix, see `redis::logger`.
    std::string log_prefix = "(Redis) ";
