...
#ifndef REDIS_ADAPTER_DETAIL_RESPONSE_TRAITS_HPP
#define REDIS_ADAPTER_DETAIL_RESPONSE_TRAITS_HPP
...
{
public:
    void operator()(std::size_t, resp3::basic_node<std::string_view> const& nd, std::error_code& ec)
    {
        switch (nd.data_type)
        {
...
public:
    ...
    void operator()(std::size_t i, resp3::basic_node<std::string_view> const& nd, std::error_code& ec)
    {
        using std::visit;
        // I am usure whether this should be an error or an assertion.
        assert(i < adapters_.size());
...
public:
    explicit vector_adapter(Vector& v) : adapter_ { internal_adapt(v) }
    {
    }
...
#endif  // REDIS_ADAPTER_DETAIL_RESPONSE_TRAITS_HPP
