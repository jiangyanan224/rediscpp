...
#ifndef REDIS_REQUEST_HPP
#define REDIS_REQUEST_HPP
...
public:
    ...
    {
        /** \brief If `true`
         * `redis::connection::async_exec` will complete with error if the
     ...
     */
    explicit request(config cfg = config { true, false, true, true }) : cfg_ { cfg }
    {
    }
    ...
    [[nodiscard]] auto get_config() const noexcept -> auto const&
    {
        return cfg_;
    }

    /// Returns a reference to the config object.
    [[nodiscard]] auto get_config() noexcept -> auto&
    {
        return cfg_;
    }
     ...
     *  \tparam Ts Non-string types will be converted to string by calling `boost_redis_to_bulk` on each argument. This function must be made available over ADL
     * and must have the following signature
     *
     *  @code
     *  void boost_redis_to_bulk(std::string& to, T const& t);
     *  {
     *     redis::resp3::boost_redis_to_bulk(to, serialize(t));
     *  }
     *  @endcode
     *
    ...
    void push_range(std::string_view const& cmd, std::string_view const& key, ForwardIterator begin, ForwardIterator end,
     ...
     *  \param begin Iterator to the begin of the range.
     *  \param end Iterator to the end of the range.
     *  \tparam ForwardIterator If the value type is not a std::string it will be converted to a string by calling `boost_redis_to_bulk`. This function must be
     * made available over ADL and must have the following signature
     *
     *  @code
     *  void boost_redis_to_bulk(std::string& to, T const& t);
     *  {
     ...
     */
    template <class ForwardIterator>
    void push_range(std::string_view const& cmd, ForwardIterator begin, ForwardIterator end,
                    typename std::iterator_traits<ForwardIterator>::value_type* = nullptr)
    {
        using value_type = typename std::iterator_traits<ForwardIterator>::value_type;
