...
#ifndef REDIS_RUNNER_HPP
#define REDIS_RUNNER_HPP

#include <redis/detail/health_checker.hpp>
...
#include <redis/detail/connector.hpp>
#include <redis/detail/resolver.hpp>
#include <redis/detail/handshaker.hpp>
...
public:
    runner_op(Runner* runner, Connection* conn, Logger l) : runner_ { runner }, conn_ { conn }, logger_ { l }
    {
    }

    template <class Self>
    void operator()(Self& self, std::array<std::size_t, 3> order = {}, std::error_code ec0 = {}, std::error_code ec1 = {}, std::error_code ec2 = {},
                    std::size_t = 0)
    {
        ASIO_CORO_REENTER(coro_)
        {
            ASIO_CORO_YIELD
            asio::experimental::make_parallel_group([this](auto token) { return runner_->async_run_all(*conn_, logger_, token); },
...
{
public:
    runner(Executor ex, config cfg) : resv_ { ex }, ctor_ { ex }, hsher_ { ex }, health_checker_ { ex }, cfg_ { cfg }
    {
    }
    ...
    {
        return asio::async_compose<CompletionToken, void(std::error_code)>(runner_op<runner, Connection, Logger> { this, &conn, l }, token, conn);
    }

    config const& get_config() const noexcept
    {
        return cfg_;
    }
...
private:
    ...
    {
        return asio::async_compose<CompletionToken, void(std::error_code)>(run_all_op<runner, Connection, Logger> { this, &conn, l }, token, conn);
    }

    template <class Connection, class Logger, class CompletionToken>
    auto async_hello(Connection& conn, Logger l, CompletionToken token)
    {
        return asio::async_compose<CompletionToken, void(std::error_code)>(hello_op<runner, Connection, Logger> { this, &conn, l }, token, conn);
    }
