...
#ifndef REDIS_HEALTH_CHECKER_HPP
#define REDIS_HEALTH_CHECKER_HPP
...
public:
    ...
    void operator()(Self& self, std::error_code ec = {}, std::size_t = 0)
    {
        ASIO_CORO_REENTER(coro_) for (;;)
        {
            ...
            REDIS_CHECK_OP0(checker_->wait_timer_.cancel();)
...
public:
    ...
    template <class Self>
    void operator()(Self& self, std::array<std::size_t, 2> order = {}, std::error_code ec1 = {}, std::error_code ec2 = {})
    {
        ASIO_CORO_REENTER(coro_)
        {
            ...
            ASIO_CORO_YIELD
            asio::experimental::make_parallel_group([this](auto token) { return checker_->async_ping(*conn_, token); },
...
public:
    ...
    template <class Connection, class CompletionToken = asio::default_completion_token_t<Executor> >
    auto async_check_health(Connection& conn, CompletionToken token = CompletionToken {})
    {
        checker_has_exited_ = false;
        return asio::async_compose<CompletionToken, void(std::error_code)>(check_health_op<health_checker, Connection> { this, &conn }, token, conn);
    }
...
private:
    ...
    {
        return asio::async_compose<CompletionToken, void(std::error_code)>(ping_op<health_checker, Connection> { this, &conn }, token, conn, ping_timer_);
    }

    template <class Connection, class CompletionToken>
    auto async_check_timeout(Connection& conn, CompletionToken token)
    {
        return asio::async_compose<CompletionToken, void(std::error_code)>(check_timeout_op<health_checker, Connection> { this, &conn }, token, conn,
                                                                           wait_timer_);
    }
...
#endif  // REDIS_HEALTH_CHECKER_HPP
