...
#ifndef REDIS_READ_HPP
#define REDIS_READ_HPP
...
public:
    append_some_op(AsyncReadStream& stream, DynamicBuffer buf, std::size_t size) : stream_ { stream }, buf_ { std::move(buf) }, size_ { size }
    {
    }
...
auto async_append_some(AsyncReadStream& stream, DynamicBuffer buffer, std::size_t size, CompletionToken&& token)
{
    return asio::async_compose<CompletionToken, void(std::error_code, std::size_t)>(append_some_op<AsyncReadStream, DynamicBuffer> { stream, buffer, size },
                                                                                    token, stream);
}

template <class AsyncReadStream, class DynamicBuffer, class ResponseAdapter>
class parse_op
{
private:
    AsyncReadStream& stream_;
...
public:
    parse_op(AsyncReadStream& stream, DynamicBuffer buf, ResponseAdapter adapter) : stream_ { stream }, buf_ { std::move(buf) }, adapter_ { std::move(adapter) }
    {
    }

    template <class Self>
    void operator()(Self& self, std::error_code ec = {}, std::size_t = 0)
    {
        ASIO_CORO_REENTER(coro_)
        {
            while (!resp3::parse(parser_, buffer_view(buf_), adapter_, ec))
            {
                ...
                ASIO_CORO_YIELD
                async_append_some(stream_, buf_, parser_.get_suggested_buffer_growth(growth), std::move(self));
                if (ec)
                {
                    self.complete(ec, 0);
                    return;
 ...
 */
template <class SyncReadStream, class DynamicBuffer, class ResponseAdapter>
auto read(SyncReadStream& stream, DynamicBuffer buf, ResponseAdapter adapter, std::error_code& ec) -> std::size_t
{
    static std::size_t const growth = 1024;
    ...
    while (!parser.done())
    {
        ...
        if (!res.has_value())
        {
            ...
            auto const n = stream.read_some(buf.data(size_before, parser.get_suggested_buffer_growth(growth)), ec);
            if (ec)
                return 0UL;
 ...
 */
template <class SyncReadStream, class DynamicBuffer, class ResponseAdapter = adapter::ignore>
auto read(SyncReadStream& stream, DynamicBuffer buf, ResponseAdapter adapter = ResponseAdapter {})
{
    std::error_code ec;
 ...
 *  the bytes after it returns.
 */
template <class AsyncReadStream, class DynamicBuffer, class ResponseAdapter = adapter::ignore,
          class CompletionToken = asio::default_completion_token_t<typename AsyncReadStream::executor_type> >
auto async_read(AsyncReadStream& stream, DynamicBuffer buffer, ResponseAdapter adapter = ResponseAdapter {},
                CompletionToken&& token = asio::default_completion_token_t<typename AsyncReadStream::executor_type> {})
{
    return asio::async_compose<CompletionToken, void(std::error_code, std::size_t)>(
        parse_op<AsyncReadStream, DynamicBuffer, ResponseAdapter> { stream, buffer, adapter }, token, stream);
}

}  // namespace redis::detail

#endif  // REDIS_READ_HPP
